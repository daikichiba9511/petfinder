...
def predict(model, test_dataloader, config, transform, fold, svr_weight=0.3, lgbm_weight=0.3):
    ...
    lgbm_preds = []
    preds = []
    # svr_model_path = f"../input/petfinder/exp005/output/exp002/svr/svr_{fold}.model"
        ...
        for fold in range(config.n_splits):
            ...
            model.load_state_dict(
                torch.load(
                    ...
                    # f"../input/petfinder/exp005/output/exp002/best_loss_{fold}.ckpt",
                    f"./output/exp005/best_loss_{fold}.ckpt",
